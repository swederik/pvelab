function project=extract_wrapper(project,TaskIndex,MethodIndex,varargin)
...
if taskidx~=0    % Common ROI file exist and applyrois method used
    ...
    ROIfile=fullfile(project.sysinfo.workspace,'r_volume_RAWATLAS');
    ROIname=fullfile(project.sysinfo.workspace,'MNI.DAT');
...
if (exist(fullfile(pn,[fn '.mat']),'file')==2)
    ...
    [res,Data]=editroiGetData(CommonRoif,VolumeFiles,[],r_GMWMf,0);
else
    project=logProject('Running voxvoiGetData. Progress is shown in matlab window...',project,TaskIndex,MethodIndex);

    [res,Data]=voxvoiGetData(fullfile(pn,[fn '.img']),VolumeFiles,[],r_GMWMf,0);
end
