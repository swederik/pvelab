function project=PVE_nogui(project,TaskIndex,MethodIndex,varargin)
...
if(isempty(project.sysinfo.logfile.name))
    [~,name,~] = fileparts(project.sysinfo.prjfile);
...
datpath=project.taskDone{taskidx}.userdata.roi.path;
datname=project.taskDone{taskidx}.userdata.roi.name;

if(isempty(project.pipeline.taskSetup{TaskIndex,MethodIndex}.configurator.default))
    settings={fullfile(datpath,datname),'Y','8.0','8.0','0.0','Y',140};
...
if isfield(project.taskDone{taskidx}.userdata,'sn')
    ...
    settings{1} = fullfile(datpath,datname);
...
if exist([pathout,'sn.mat'], 'file')==0
    ...
    try
        try
            seg1path=project.taskDone{taskidx-2}.userdata.segoutReslice{1}.path;
            seg1name=project.taskDone{taskidx-2}.userdata.segoutReslice{1}.name;
        catch
            seg1path=pathout;
...
[~,roiName,roiExt]=fileparts(settings{1});
