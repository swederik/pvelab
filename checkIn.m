function [project,stateReturn]=checkIn(project,TaskIndex,MethodIndex,varargin)
...
if(ishandle(project.handles.h_mainfig) )
    ...
    %Update data stored in mainfig   
    set(project.handles.h_mainfig,'UserData',project);       
    %Configurator GUI is not a part of the pipeline program, therefor no update
    if(~chkInConfigurator)&& ~strcmpi(project.pipeline.taskSetup{TaskIndex,1}.task,'others')
        drawGUI(project.handles.h_mainfig,TaskIndex,'updateFanestatus');
    end
end