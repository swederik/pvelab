function project=setupProject(handles,UsePipeline,varargin)
...
tmpTask.function_wrapper='main_wrapper';
...
%__________ Setup handles-entry
if(isfield(handles,'h_mainfig'))
    set(handles.h_mainfig,'name',sprintf('PIPELINE: %s',pipeline.taskSetup{end,1}.method_name));% Set titlebar to reference name 
else
    clear('handles');
    handles.h_mainfig=figure('visible','off'); %No handle exist then create one...
    set(handles.h_mainfig,'name',sprintf('PIPELINE: %s',pipeline.taskSetup{end,1}.method_name));
end

if(isfield(handles,'h_logwin'))
    handles.h_logwin=handles.h_logwin;
else
    handles.h_logwin=[];
end

if(isfield(handles,'h_display'))
    handles.h_display=handles.h_display;
else
    handles.h_display=[];
end
...
project.handles=handles;

%_______ Save project in mainfig if exist
if(isfield(handles,'h_mainfig'))
   set(handles.h_mainfig,'UserData',project)
end
