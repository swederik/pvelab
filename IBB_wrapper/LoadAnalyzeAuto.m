        ...
        switch task
            ...
            otherwise
                ...
                if exist(task)==2
                    ...
                    hdrfile=[Path filesep File '.hdr'];
                    imgfile=[Path filesep File '.img'];
                    if exist(hdrfile)==2
                        if exist(imgfile)==2
...
if length(hdr.dim)==4 & hdr.dim(4)>1 % Multi-frame 
    ...
    if Ok==0
        ...
        if Choice>1 & Choice < lc
            if RawFlag==1
                img=ReadAnalyzeImg([Path filesep File],[':' num2str(Choice-1)],'raw');
            else
                img=ReadAnalyzeImg([Path filesep File],[':' num2str(Choice-1)]);
            end
            hdr.dim=hdr.dim(1:3);
        elseif Choice==lc
            for j=1:hdr.dim(4)
                if RawFlag==1
                    img{j}=ReadAnalyzeImg([Path filesep File],[':' num2str(j)],'raw');
                else
                    img{j}=ReadAnalyzeImg([Path filesep File],[':' num2str(j)]);
                end
            end
            hdr.dim=hdr.dim(1:3);
        elseif Choice==1
            if not(hdr.pre==16 | hdr.pre==8 | hdr.pre==32)
                ...
                for j=range(1):range(2)
                    ...
                    if j==range(1);
                        img=double(ReadAnalyzeImg([Path filesep File],[':' num2str(j)]));
                    else
                        img=img+double(ReadAnalyzeImg([Path filesep File],[':' num2str(j)]));
                    end  
                end;	  
                if (hdr.pre==16)
                    MaxImg=max(img(:));
    ...
    if RawFlag==1
        img=ReadAnalyzeImg([Path filesep File],'raw');
    else
        img=ReadAnalyzeImg([Path filesep File]);
    end
end
