function project=IMGEdit_wrapper(project,TaskIndex,MethodIndex,varargin)
...
if time>1,
    error('IMGEdit: multi-frame images are not supported.');
    return
end
...
f1=f(3)-20;
f2=f(4)-80;
fig=figure('units','pixels','position',[10 40 f1 f2],'MenuBar','none','NumberTitle','off','Name','IMGEdit','Color',color);
...
uicontrol('parent',fig,'style','pushbutton','units','pixels','position',[f1-115 f2-140 80 25],'HorizontalAlignment','center','string','Modify sel.','Callback',{@ModifySEL});
...
%--------------------------------------------------------------------
function ModifySEL(h, eventdata)
global fig;
global currslice;
global selectedpoints;
global u1;
