function project=SpmCoregAuto_wrapper(project,TaskIndex,MethodIndex,varargin)
% SW version: 260903TR
% SpmCoregAuto_wrapper calls SPM2 or SPM5 routines to perform coregistration.
%
% Input:
...
switch exist('spm','file')
    ...
    case 2
        [v,c]=spm('Ver');
        if (strcmpi(v,'spm2'))
            project=logProject('SpmCoregAuto_wrapper: SPM2 found. Co-registration progress is shown in SPM-windows.',project,TaskIndex,MethodIndex);
            SPM_ver=2;
        elseif (strcmpi(v,'spm5'))
            project=logProject('SpmCoregAuto_wrapper: SPM5 found. Co-registration progress is shown in SPM-windows.',project,TaskIndex,MethodIndex);
            SPM_ver=5;
        elseif(strcmpi(v,'spm8'))
            project=logProject('SpmCoregAuto_wrapper: SPM8 found. Co-registration progress is shown in SPM-windows.',project,TaskIndex,MethodIndex);
            SPM_ver=8;
        else
            msg='SpmCoregAuto_wrapper: Wrong SPM version, SPM2/5/8 required. Download SPM at www.fil.ion.ucl.ac.uk/spm.';
            project=logProject(msg,project,TaskIndex,MethodIndex);
            project.taskDone{TaskIndex}.error{end+1}=msg;
            return;
        end
...
if (SPM_ver==2)||(SPM_ver==5)
    ...
    if(isempty(project.pipeline.taskSetup{TaskIndex,MethodIndex}.configurator.default))
        ...
        %loginfo
        msg='SpmCoregAuto_wrapper: Using SPM2 or SPM5 default settings defined in spm_defaults.m.';
    end
    ...
    if(isempty(project.pipeline.taskSetup{TaskIndex,MethodIndex}.configurator.user))
        ...
        msg='SpmCoregAuto_wrapper: User settings are loaded';
    end;
    project=logProject(msg,project,TaskIndex,MethodIndex);
    ...
    if (userConfig.to==1) %Select object and target picture from config.
        ...
        if length(PEThdr.dim)>3 && PEThdr.dim(4)>1
            ...
            [pn,fn]=fileparts(sourceFile1);
            sourceFile1Mean=fullfile(pn,[fn '_mean.img']);
            if exist(sourceFile1Mean,'file')==2    % Testing if _mean image exist and then use that
                [pnm,fnm]=fileparts(sourceFile1Mean);
                %copyfile(fullfile(pnm,[fnm '.img']),fullfile(project.sysinfo.workspace,[fnm '.img']));
                %copyfile(fullfile(pnm,[fnm '.hdr']),fullfile(project.sysinfo.workspace,[fnm '.hdr']));
                
                substi=sourceFile1Mean;
                targImg=sourceFile1Mean;            % Has to redo this because object is automatically changed to _mean image
                [targPath,targName]=fileparts(targImg);
                
                %Register used pet image in project structure
                % Already registered in FileLoad1Auto
                %project.taskDone{TaskIndex}.userdata.usedPET{ImageIndex,1}.path=project.sysinfo.workspace;
                %project.taskDone{TaskIndex}.userdata.usedPET{ImageIndex,1}.name=[fnm '.img'];
                %project.taskDone{TaskIndex}.userdata.usedPET{ImageIndex,1}.info='PET used for co-registration.';
            else
                %[petimg,pethdr]=LoadAnalyzeAuto(sourceFile1,'single');
                [petimg,pethdr]=LoadAnalyze(sourceFile1,'all');
    ...
    elseif (userConfig.to==2)
        ...
        if length(PEThdr.dim)>3 && PEThdr.dim(4)>1
            ...
            if exist(sourceFile1Mean,'file')==2    % Testing if _mean image exist and then use that
                ...
                substi=sourceFile1Mean;
                objImg=sourceFile1Mean;            % Has to redo this because object is automatically changed to _mean image
                [objPath,objName]=fileparts(targImg);
...
elseif (SPM_ver==8)
    ...
    if(isempty(project.pipeline.taskSetup{TaskIndex,MethodIndex}.configurator.default))
        ...
        %loginfo
        msg='SpmCoregAuto_wrapper: Using SPM8 default settings defined in spm_defaults.m.';
    end
    ...
    if (userConfig.to==2) % Only works for co-registration to MR in spm8
        ...
        if length(PEThdr.dim)>3 && PEThdr.dim(4)>1
            ...
            if exist(sourceFile1Mean,'file')==2    % Testing if _mean image exist and then use that
                ...
                substi=sourceFile1Mean;
                objImg=sourceFile1Mean;            % Has to redo this because object is automatically changed to _mean image
                [objPath,objName]=fileparts(objImg);
    ...
    project=logProject(['SpmCoregAuto_wrapper: Created SPM8 batch file: ',Spm8CoregBatchFile],project,TaskIndex,MethodIndex);
    
    % _________________________ Start co-registration ________________________
    project=logProject('SpmCoregAuto_wrapper: SPM8 Co-registration T1-weighted MRI.',project,TaskIndex,MethodIndex);
    spm_jobman('initcfg');
